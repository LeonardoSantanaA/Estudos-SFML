...
//TODO transformar em orientacao a objetos
...
void bullet(std::shared_ptr<sf::Sprite> bullet, std::shared_ptr<sf::Sprite> spaceship, std::shared_ptr<sf::Texture> bullet_t,
    ...
    float &frame, float &speedFrame, std::shared_ptr<sf::Sprite> explosion_spr, float &max, float &count, bool &show){
  if(shoot < 12)
    ++shoot;
...
int main(){
  ...
  while(window->isOpen()){
    ...
    bullet(bullet_spr, spaceship_spr, bullet_t, spaceship_t, bullets, shoot, enemies, frame, speedFrame, explosion_spr, max, count, show);
    ...
    //vou fazer uma condicao ternaria, para desenhar ou o sprite da vida ou gameover
